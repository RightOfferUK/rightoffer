...
export async function GET(
  ...
  try {
    ...
    // Verify the current user is a real estate admin
    const currentUser = await User.findById(session.user.id).lean() as UserDocument | null;
    if (!currentUser || currentUser.role !== 'real_estate_admin') {
    ...
    // Build query
    const query: Record<string, unknown> = { agentId: new mongoose.Types.ObjectId(agentId) };
